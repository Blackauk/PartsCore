...
import { mockReceives } from '../../data/mockMovements.js';
...
export default function Receive() {
  ...
  // Safe defaults
  const receives = Array.isArray(mockReceives) ? mockReceives : [];
  ...
  const sorted = useMemo(() => {
    if (!sortKey || !sortDir) return receives;
    const copy = [...receives];
    copy.sort((a, b) => {
      let va = a[sortKey];
      let vb = b[sortKey];
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((r) => {
      ...
      const poNo = (r.poNo || '').toLowerCase();
      return supplier.includes(q) || partNo.includes(q) || poNo.includes(q);
    });
  }, [search, sorted]);
  ...
  function handleExport() {
    ...
    if (rows.length === 0) {
      const templateHeaders = ['date','supplier','poNo','partNo','description','qtyReceived','location','receivedBy','status'];
      exportToCSV('receive_template.csv', templateHeaders, []);
    } else {
      exportToCSV('receive_data.csv', headers, rows);
    }
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <TableCard 
        ...
        onSort={handleSort}
      />
      <EditModal open={!!editing} onClose={() => setEditing(null)} row={editing} title="Edit Receipt" />
      <CSVModal
        open={importOpen}
