...
import { mockIssues } from '../../data/mockMovements.js';
...
export default function Issue() {
  ...
  const sorted = useMemo(() => {
    if (!sortKey || !sortDir) return issues;
    const copy = [...issues];
    copy.sort((a, b) => {
      let va = a[sortKey];
      let vb = b[sortKey];
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((r) => {
      if (!r) return false;
      if (!q) return true;
      const issuedTo = (r.issuedTo || '').toLowerCase();
      const partNo = (r.partNo || '').toLowerCase();
      const jobPlant = (r.jobPlant || '').toLowerCase();
      return issuedTo.includes(q) || partNo.includes(q) || jobPlant.includes(q);
    });
  }, [search, sorted]);
  ...
  function handleExport() {
    ...
    if (rows.length === 0) {
      const templateHeaders = ['date','issuedTo','jobPlant','partNo','description','qty','issuedBy','status'];
      exportToCSV('issue_template.csv', templateHeaders, []);
    } else {
      exportToCSV('issue_data.csv', headers, rows);
    }
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <TableCard 
        ...
        onSort={handleSort}
      />
      <EditModal open={!!editing} onClose={() => setEditing(null)} row={editing} title="Edit Issue" />
