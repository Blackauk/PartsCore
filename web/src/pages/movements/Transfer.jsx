...
import { mockTransfers } from '../../data/mockMovements.js';
...
export default function Transfer() {
  ...
  // Safe defaults
  const transfers = Array.isArray(mockTransfers) ? mockTransfers : [];
  ...
  const sorted = useMemo(() => {
    if (!sortKey || !sortDir) return transfers;
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((r) => {
      ...
      return from.includes(q) || to.includes(q) || partNo.includes(q);
    });
  }, [search, sorted]);
  ...
  function handleExport() {
    ...
    if (rows.length === 0) {
      const templateHeaders = ['date','from','to','partNo','description','qty','by','status'];
      exportToCSV('transfer_template.csv', templateHeaders, []);
    } else {
      exportToCSV('transfer_data.csv', headers, rows);
    }
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <TableCard 
        ...
        onSort={handleSort}
      />
      <EditModal open={!!editing} onClose={() => setEditing(null)} row={editing} title="Edit Transfer" />
